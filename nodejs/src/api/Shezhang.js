...
import ShezhangModel from '../models/ShezhangModel'
...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await ShezhangModel.findOne({ where: { shezhangzhanghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.shezhangzhanghao,
					role: '社长',
                    loginUserColumn: 'shezhangzhanghao',
                    tableName: 'shezhang'
				},
				config.jwtSecret,
				{
			...
			userinfo.dataValues.role = '社长'
			userinfo.dataValues.loginUserColumn = 'shezhangzhanghao'
			userinfo.dataValues.tableName = 'shezhang'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			const userinfo = await ShezhangModel.create(req.body)
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ShezhangModel.findOne({ where: { id: req.session.userinfo === undefined ? jwt.decode(req.headers.token).id : req.session.userinfo.id } }))
		} catch(err) {
	...
	api.all('/resetPass', async (req, res) => {
    	...
    	let pwd = '123456'
			...
			await ShezhangModel.update({
				mima: pwd
			}, {
				where: {
				  shezhangzhanghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let shezhangxingming = req.query.shezhangxingming
			if (shezhangxingming) {
			...
			let result = await ShezhangModel.findAndCountAll({
				order: [[sort, order]],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await ShezhangModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ShezhangModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM shezhang WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM shezhang WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM shezhang WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM shezhang WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM shezhang WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM shezhang WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "shezhang"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM shezhang " + where + " GROUP BY " + columnName + " LIMIT 10" 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM shezhang " + where + " GROUP BY " + xColumnName + " DESC LIMIT 10"
