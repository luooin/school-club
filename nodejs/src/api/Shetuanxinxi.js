...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let shetuanxuanyan = req.query.shetuanxuanyan
			if (shetuanxuanyan) {

				if (shetuanxuanyan.indexOf('%') != -1) {
					where.shetuanxuanyan = {
						[Op.like]: shetuanxuanyan
					}
				} else {
					where.shetuanxuanyan = {
						[Op.eq]: shetuanxuanyan
					}
				}
			}
			let shetuanjieshao = req.query.shetuanjieshao
			if (shetuanjieshao) {

				if (shetuanjieshao.indexOf('%') != -1) {
					where.shetuanjieshao = {
						[Op.like]: shetuanjieshao
					}
				} else {
					where.shetuanjieshao = {
						[Op.eq]: shetuanjieshao
					}
				}
			}
			...
			let shetuantupian = req.query.shetuantupian
			if (shetuantupian) {

				if (shetuantupian.indexOf('%') != -1) {
					where.shetuantupian = {
						[Op.like]: shetuantupian
					}
				} else {
					where.shetuantupian = {
						[Op.eq]: shetuantupian
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
            ...
            await ShetuanxinxiModel.update({
			...
			const recordInfo = await ShetuanxinxiModel.findOne({ where: { id: req.params.id } })
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM shetuanxinxi " + where + " GROUP BY " + columnName + " LIMIT 10" 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM shetuanxinxi " + where + " GROUP BY " + xColumnName + " DESC LIMIT 10"
